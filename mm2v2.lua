--[[
   ...
   All YARHM references replaced with ASTROSTAR.
   Added: Dropdown fling target, auto-shoot uncommented, role labels.
]]
...
Converted["_Notifications"].Parent = Converted["_ASTROSTAR"]
...
Converted["_AddCustomModule"].Parent = Converted["_ASTROSTAR"]
	...
	local function module_script()
		...
		function FUNCTIONSmodule.notification(s, color, icon)
			...
			task.spawn(function()
				...
				notif.Visible=true
				notif.Name=s
				if color and typeof(icon)=="Color3" then notif.UIStroke.Color=color; notif.ImageLabel.ImageColor3=color end
				if icon then if icons[icon] then notif.ImageLabel.Image=icons[icon] else if tonumber(icon) then notif.ImageLabel.Image="rbxassetid://"..tonumber(icon) else notif.ImageLabel.Image=icon end end end
		...
		function FUNCTIONSmodule.finetuneFloatingButton(button, dragger)
			...
			for _,b in ipairs(script.Parent.FloatingButtons:GetChildren()) do if b:IsA("TextButton") and b:FindFirstChildWhichIsA("UIScale") then local buttonScale=b:FindFirstChildWhichIsA("UIScale"); ts:Create(buttonScale, TweenInfo.new(0.3), {Scale=0}):Play() end end
			...
			fBSFResizeDragger.Dragged=function(pos)
				...
				button.Size=newSize
				ASTROSTARPointSave:set(string.gsub(button.Name,"_",""), udim2Serializer(button.Position).."|"..udim2Serializer(button.Size).."|"..tostring(button.Visible).."|"..tostring(dragger.CanBeDragged))
			end
		...
		function FUNCTIONSmodule.createFloatingButton(item,button,buttonname,fromload)
			if not getgenv().ASTROSTAR.FloatingButtons:FindFirstChild(string.gsub(buttonname,"_","")) then
				...
				if not fromload then ASTROSTARPointSave:set(string.gsub(buttonname,"_",""), udim2Serializer(UDim2.fromOffset(125,90)).."|"..udim2Serializer(UDim2.fromOffset(200,50)).."|true|true") end
				...
				local themedColor=Instance.new("StringValue",newFloatingButton); themedColor.Name="themedColor"; themedColor.Value="primaryColor"
				...
				newFloatingButton.MouseButton1Click:Connect(function()
					if typeof(item["Args"][2])=="function" then item["Args"][2](button) else item["Args"][2][buttonname](button) end
				end)
				local ripple
				...
				local function closeRipple()
					...
					if ripple then task.spawn(function() local rippleToRemove=ripple; local fade=ts:Create(rippleToRemove, TweenInfo.new(0.5, Enum.EasingStyle.Cubic, Enum.EasingDirection.Out), {BackgroundTransparency=1, Size=UDim2.fromOffset(150,150)}); fade:Play(); fade.Completed:Once(function() rippleToRemove:Destroy() end) end) end
				end
				UserInputService.InputEnded:Connect(function(input) if input.UserInputType==Enum.UserInputType.MouseButton1 or input.UserInputType==Enum.UserInputType.Touch then closeRipple() end end)
				local shouldBeDraggable=true
				if not fromload then
					newFloatingButton.Position=UDim2.fromOffset(-125,90)
				elseif ASTROSTARPointSave:get(string.gsub(buttonname,"_","")) then
					local data=ASTROSTARPointSave:get(string.gsub(buttonname,"_","")):split("|")
				...
				task.spawn(function() if not fromload then ts:Create(newFloatingButton, TweenInfo.new(2, Enum.EasingStyle.Elastic, Enum.EasingDirection.Out), {Size=UDim2.fromOffset(200,50)}):Play(); ts:Create(newFloatingButton, TweenInfo.new(0.7, Enum.EasingStyle.Cubic, Enum.EasingDirection.Out), {Position=UDim2.fromOffset(125,90)}):Play() end end)
				...
				floatingButtonDraggers[string.gsub(buttonname,"_","")].Dragged=function(newPos)
					ASTROSTARPointSave:set(string.gsub(buttonname,"_",""), udim2Serializer(newPos).."|"..udim2Serializer(newFloatingButton.Size).."|"..tostring(newFloatingButton.Visible).."|"..tostring(floatingButtonDraggers[string.gsub(buttonname,"_","")].CanBeDragged))
				end
				...
				newFloatingButton.InputBegan:Connect(function(input)
					if input.UserInputType==Enum.UserInputType.MouseButton2 then
						...
						repeat result=UserInputService.InputBegan:Wait(); if result.UserInputType==Enum.UserInputType.Keyboard then keytobind=result.KeyCode end until keytobind
				...
				ASTROSTARPointSave:remove(string.gsub(buttonname,"_",""))
				task.spawn(function()
					local buttontodestroy=getgenv().ASTROSTAR.FloatingButtons:FindFirstChild(string.gsub(buttonname,"_",""))
					...
					btdtween:Play(); btdtween.Completed:Wait(); buttontodestroy:Destroy()
				end)
			end
		end
			...
			for _,module in ipairs(getgenv().Modules) do
				for _,item in ipairs(module) do
					if item["Type"]=="Button" then
						...
						if ASTROSTARPointSave:get(key) then FUNCTIONSmodule.createFloatingButton(item, Instance.new("TextButton"), item["Args"][1], true) end
					end
				end
			end
		end
		...
		function FUNCTIONSmodule.loader(module)
			...
			for _,item in ipairs(module) do
				...
				elseif item["Type"]=="Button" then
					...
					local padding=Instance.new("UIPadding"); padding.Parent=button; padding.PaddingTop=UDim.new(0,5); padding.PaddingBottom=UDim.new(0,5)
				...
				elseif item["Type"]=="Input" then
					...
					cloneinput.Parent=frameHolder; cloneinput.Visible=true
				...
				elseif item["Type"]=="Toggle" then
					...
					clonetoggle.Parent=frameHolder; clonetoggle.Visible=true
					...
					clonetoggletoggler.MouseButton1Click:Connect(function()
						if toggleStates[item["Args"][1]..module.Name] then
							...
							ts:Create(clonetoggletoggler, TweenInfo.new(0.5, Enum.EasingStyle.Cubic, Enum.EasingDirection.Out), {Position=UDim2.fromScale(0.3,0.5)}):Play()
							clonetoggletoggler.ImageLabel.Image="rbxassetid://10002373478"
						else
							toggleStates[item["Args"][1]..module.Name]=true
							ts:Create(clonetoggletoggler, TweenInfo.new(0.5, Enum.EasingStyle.Cubic, Enum.EasingDirection.Out), {Position=UDim2.fromScale(0.7,0.5)}):Play()
							clonetoggletoggler.ImageLabel.Image="rbxassetid://5959696880"
						end
				...
				elseif item["Type"]=="Dropdown" then
					...
					clonedropdown.Parent=frameHolder; clonedropdown.Visible=true
					...
					clonedropdown.Frame.MouseButton1Click:Connect(function()
						...
						for _,v in ipairs(items) do
							...
							clonedropdownbutton.MouseButton1Click:Connect(function()
								...
								after:Play(); after.Completed:Once(function() dropdownFrame.Visible=false end)
							end)
						end
					end)
				elseif item["Type"]=="Range" then
					local clonerange=getgenv().ASTROSTAR.Range:Clone()
					clonerange.Parent=frameHolder; clonerange.Visible=true
			...
			ts:Create(listlayout, TweenInfo.new(0.5, Enum.EasingStyle.Exponential, Enum.EasingDirection.Out), {Padding=UDim.new(0,10)}):Play()
			local AREAframes={}
			for _,i in ipairs(AREA:GetChildren()) do if i:IsA("Frame") then table.insert(AREAframes,i) end end
			if #AREAframes>5 then
		...
		function FUNCTIONSmodule.refreshlist()
			...
			for i,module in ipairs(dense) do
				local success,err=pcall(function()
					...
					local themedColor=Instance.new("StringValue",listbutton); themedColor.Name="themedColor"; themedColor.Value="primaryColor"
...
local function WMYX_routine()
	...
	local espindc=require(script.Parent.ESPIndicator)
	local espcontainer=espindc.new({ArrowEdgePadding=50, ArrowShowDistanceText=false})
	module.players=false; module.pcs=false; module.pods=false; module.exits=false; module.lockers=false; module.antipcerror=false; module.flashlight=false
	local hideLabelsAndArrows=false
	local function getBeast()
		for _,player in ipairs(game.Players:GetChildren()) do
			if player.Character and player.Character:FindFirstChild("BeastPowers") then return player end
		end
	end
			...
			for _,obj in ipairs(workspace:GetDescendants()) do
				if obj.Name=="ComputerTable" then
					if obj.Screen.Color==Color3.fromRGB(40,127,71) then
						espcontainer:Add(obj, {AccentColor=Color3.new(0.133333,0.333333,0.00784314), ArrowShow=false, ArrowShowDistanceText=false, ShowLabel=false, GroupName="pcs"})
					else
						espcontainer:Add(obj, {AccentColor=Color3.new(0,0.37,1), ArrowShow=not hideLabelsAndArrows, ArrowMinDistance=99999, ArrowShowDistanceText=false, ArrowSize=UDim2.new(0,10,0,10), ShowLabel=false, GroupName="pcs"})
					end
				end
			end
		end
	...
	table.insert(module,{Type="ButtonGrid",Toggleable=true,Args={3,{
		Players=function(Self) module.players=not module.players; reloadESP() end,
		PCs=function(Self) module.pcs=not module.pcs; reloadESP() end,
		Pods=function(Self) module.pods=not module.pods; reloadESP() end,
		Exits=function(Self) module.exits=not module.exits; reloadESP() end,
		Lockers=function(Self) module.lockers=not module.lockers; reloadESP() end,
	}}})
	...
	local light=Instance.new("PointLight",root); light.Brightness=0; light.Range=9999999999
	local wslock=false; local ws=18; local antifail=false
	task.spawn(function()
		if game:GetService("RunService"):IsStudio() then return end
		local OldNameCall=nil
		OldNameCall=hookmetamethod(game,"__namecall",function(Self,...)
			local Args={...}; local NamecallMethod=getnamecallmethod()
			if NamecallMethod=="FireServer" and Args[1]=="SetPlayerMinigameResult" and antifail then
				print("Minigame result - Intercepting result to true"); Args[2]=true
			end
			return OldNameCall(Self,unpack(Args))
		end)
	end)
	table.insert(module,{Type="ButtonGrid",Toggleable=true,Args={3,{
		Anti_PC_Error=function() antifail=not antifail end,
		Flashlight=function() light.Brightness= light.Brightness==0 and 2.5 or 0 end,
	}}})
	task.spawn(function() while task.wait(0.1) do if wslock then root.Parent:WaitForChild("Humanoid").WalkSpeed=ws end end end)
	table.insert(module,{Type="Input",Args={"Input a walkspeed","Set & Lock",function(Self,text) if not tonumber(text) then FUNCTIONS.notification("Input isn't a valid number.") return end; ws=tonumber(text); wslock=true end}})
	table.insert(module,{Type="Button",Args={"Unlock all",function() wslock=false end}})
	table.insert(module,{Type="Text",Args={"Locking means your speed will stay the same no matter what. This means you will not be slow when crawling or jumping as beast."}})
	repeat task.wait() until getgenv().Modules
...
local function CEBY_routine()
	...
	local module={}
	module["gameId"]=0
	if (module["gameId"]~=game.GameId) and module["gameId"]~=0 then script.Enabled=true end
	local ts=game:GetService("TweenService")
	local uis=game:GetService("UserInputService")
	local rs=game:GetService("RunService")
	local https=game:GetService("HttpService")
	...
	local loopfovandws=false; local ctrlclicktp=false; local ws=16; local fov=70; local hidden=false
	local ASTROSTARPointSave=PointSave.new("ASTROSTAR")
	function splitString(str,delim) local broken={} if delim==nil then delim="," end for w in string.gmatch(str,"[^"..delim.."]+") do table.insert(broken,w) end return broken end
	function toTokens(str) local tokens={} for op,name in string.gmatch(str,"([+-])([^+-]+)") do table.insert(tokens,{Operator=op,Name=name}) end return tokens end
	function onlyIncludeInTable(tab,matches) local matchTable={} local resultTable={} for i,v in pairs(matches) do matchTable[v.Name]=true end for i,v in pairs(tab) do if matchTable[v.Name] then table.insert(resultTable,v) end end return resultTable end
	function removeTableMatches(tab,matches) local matchTable={} local resultTable={} for i,v in pairs(matches) do matchTable[v.Name]=true end for i,v in pairs(tab) do if not matchTable[v.Name] then table.insert(resultTable,v) end end return resultTable end
	function getPlayersByName(Name) local Name,Len,Found=string.lower(Name),#Name,{} for _,v in pairs(Players:GetPlayers()) do if Name:sub(0,1)=='@' then if string.sub(string.lower(v.Name),1,Len-1)==Name:sub(2) then table.insert(Found,v) end else if string.sub(string.lower(v.Name),1,Len)==Name or string.sub(string.lower(v.DisplayName),1,Len)==Name then table.insert(Found,v) end end end return Found end
	function getPlayer(list,speaker) if list==nil then return {speaker.Name} end local nameList=splitString(list,",") local foundList={} for _,name in pairs(nameList) do if string.sub(name,1,1)~="+" and string.sub(name,1,1)~="-" then name="+"..name end local tokens=toTokens(name) local initialPlayers=Players:GetPlayers() for i,v in pairs(tokens) do if v.Operator=="+" then local tokenContent=v.Name local foundCase=false if not foundCase then initialPlayers=onlyIncludeInTable(initialPlayers,getPlayersByName(tokenContent)) end else local tokenContent=v.Name local foundCase=false if not foundCase then initialPlayers=removeTableMatches(initialPlayers,getPlayersByName(tokenContent)) end end end for i,v in pairs(initialPlayers) do table.insert(foundList,v) end end local foundNames={} for i,v in pairs(foundList) do table.insert(foundNames,v.Name) end return foundNames[1] end
	task.spawn(function() rs.RenderStepped:Connect(function() if loopfovandws then workspace.CurrentCamera.FieldOfView=fov; if game.Players.LocalPlayer.Character and game.Players.LocalPlayer.Character:FindFirstChild("Humanoid") then game.Players.LocalPlayer.Character.Humanoid.WalkSpeed=ws end end end) end)
	uis.InputBegan:Connect(function(inp,proc) if proc then return end if uis:IsKeyDown(Enum.KeyCode.LeftControl) and inp.KeyCode==Enum.KeyCode.Y and hidden then hidden=false; ts:Create(script.Parent.Menu.UIScale, TweenInfo.new(1, Enum.EasingStyle.Cubic, Enum.EasingDirection.Out), {Scale=1}):Play() end end)
	local function getPlayerMouse() local player=game:GetService("Players").LocalPlayer if player then return player:GetMouse() end return nil end
	local function getRayHitPosition() local mouse=getPlayerMouse() if not mouse then return nil end local camera=workspace.CurrentCamera local unitRay=camera:ScreenPointToRay(mouse.X,mouse.Y) local ray=Ray.new(unitRay.Origin, unitRay.Direction*1000) local part,position=workspace:FindPartOnRay(ray, game:GetService("Players").LocalPlayer.Character) if part then return position else return nil end end
	uis.InputBegan:Connect(function(inp,proc) if proc then return end if uis:IsKeyDown(Enum.KeyCode.LeftControl) and inp.UserInputType==Enum.UserInputType.MouseButton1 and ctrlclicktp then local ray=getRayHitPosition() if not ray then fu.notification("Couldn't find a place to teleport to.") return end game.Players.LocalPlayer.Character:WaitForChild("HumanoidRootPart").CFrame=CFrame.new(ray) end end)
	if uis.AccelerometerEnabled then uis.DeviceAccelerationChanged:Connect(function(acc) if hidden and acc.Position.Magnitude>28 then hidden=false; ts:Create(script.Parent.Menu.UIScale, TweenInfo.new(1, Enum.EasingStyle.Cubic, Enum.EasingDirection.Out), {Scale=1}):Play() end end) end
	module["Name"]="Universal"
	table.insert(module,{Type="Text",Args={"<font color='#FFFF00'>Another great script</font> by ASTRO STAR developers below!"}})
	table.insert(module,{Type="Button",Args={"AFEM Max - The best AI-powered emote script!", function() loadstring(game:HttpGet("https://yarhm.mhi.im/scr?channel=afemmax"))(); fu.notification("AFEM has been executed.") end}})
	table.insert(module,{Type="Text",Args={"---"}})
	table.insert(module,{Type="Button",Args={"Join our Discord", function(Self) if setclipboard then setclipboard("https://discord.gg/2jbYxvDkxr") end; fu.notification('Discord link has been copied to clipboard!') end}})
	table.insert(module,{Type="Toggle",Args={"OP Fly", function(Self,state) if state then flyutility:Start(Players.LocalPlayer.Character) else flyutility:Stop(Players.LocalPlayer.Character) end end}})
	table.insert(module,{Type="Range",Args={"Fly speed",50,350,10,function(Self,spd) flyutility:SetMaxSpeed(spd) end}})
	local infJump=false; local infJumpConnection=nil; local landedConnection=nil; local infJumps=0; local infJumpDeb=false; local infJumpOnlyTwo=false; local landed=true
	local function setupHumanoid(humanoid) if landedConnection then landedConnection:Disconnect() end; landedConnection=humanoid.StateChanged:Connect(function(_,n) if n==Enum.HumanoidStateType.Landed or n==Enum.HumanoidStateType.Running then landed=true; infJumps=0 end end) end
	table.insert(module,{Type="Toggle",Args={"Infinite jump", function(Self,state) infJump=state; if state then local char=game.Players.LocalPlayer.Character if char and char:FindFirstChildWhichIsA("Humanoid") then setupHumanoid(char:FindFirstChildWhichIsA("Humanoid")) end; infJumpConnection=uis.JumpRequest:Connect(function() local character=game.Players.LocalPlayer.Character local humanoid=character and character:FindFirstChildWhichIsA("Humanoid") if not humanoid then return end; if infJumpOnlyTwo and infJumps>=2 and not landed then return end; if not infJumpDeb then infJumpDeb=true; humanoid:ChangeState(Enum.HumanoidStateType.Jumping); infJumps=infJumps+1; landed=false; task.wait(.1); infJumpDeb=false end end) else if infJumpConnection then infJumpConnection:Disconnect() end; if landedConnection then landedConnection:Disconnect() end; infJumps=0; landed=true end end}})
	table.insert(module,{Type="Toggle",Args={"Limit infinite jump to 2 jumps only", function(Self,state) infJumpOnlyTwo=state; infJumps=0 end}})
	local aggressiveExp=false; local hitboxExp=1
	table.insert(module,{Type="Input",Args={"Hitbox expander","Expand everyone's hitbox",function(Self,ToExpand) hitboxExp=ToExpand; local players=game:GetService("Players"):GetPlayers() for i,v in ipairs(players) do if v~=game.Players.LocalPlayer and v.Character:FindFirstChild('HumanoidRootPart') then local sizeArg=tonumber(ToExpand); local Size=Vector3.new(sizeArg,sizeArg,sizeArg); if aggressiveExp then for _,part in ipairs(v.Character:GetChildren()) do if part:IsA("BasePart") then if not ToExpand or sizeArg==1 then part.Size=Vector3.new(2,1,1); part.Transparency=0.2 else part.Size=Size; part.Transparency=0.2 end end end else local Root=v.Character:FindFirstChild('HumanoidRootPart') if Root:IsA("BasePart") then if not ToExpand or sizeArg==1 then Root.Size=Vector3.new(2,1,1); Root.Transparency=0.2 else Root.Size=Size; Root.Transparency=0.2 end; Root.CanCollide=false end end end end; fu.notification("Hitboxes expanded.") end}})
	local loopHitBoxExp; table.insert(module,{Type="Toggle",Args={"Loop hitbox expansion", function(Self,state) if state then loopHitBoxExp=rs.Heartbeat:Connect(function() local players=game:GetService("Players"):GetPlayers() for i,v in ipairs(players) do if v~=game.Players.LocalPlayer and v.Character:FindFirstChild('HumanoidRootPart') then local sizeArg=tonumber(hitboxExp); local Size=Vector3.new(sizeArg,sizeArg,sizeArg); local Root=v.Character:FindFirstChild('HumanoidRootPart'); if aggressiveExp then for _,part in ipairs(v.Character:GetChildren()) do if part:IsA("BasePart") then if not hitboxExp or sizeArg==1 then part.Size=Vector3.new(2,1,1); part.Transparency=0.2 else part.Size=Size; part.Transparency=0.2 end end end else local Root=v.Character:FindFirstChild('HumanoidRootPart') if Root:IsA("BasePart") then if not hitboxExp or sizeArg==1 then Root.Size=Vector3.new(2,1,1); Root.Transparency=0.2 else Root.Size=Size; Root.Transparency=0.2 end; Root.CanCollide=false end end end end end) else loopHitBoxExp:Disconnect() end end}})
	table.insert(module,{Type="Toggle",Args={"Aggressive hitbox expasion (all parts)", function(Self,state) aggressiveExp=state end}})
	table.insert(module,{Type="Input",Args={"Walkspeed","Set speed",function(Self,speed) local lp=game:GetService("Players").LocalPlayer; local char=lp.Character; if not char then fu.notification("No character!") return end; local hu=char:FindFirstChildOfClass("Humanoid"); if not hu then fu.notification("No humanoid on your character..?") return end; hu.WalkSpeed=tonumber(speed) or 16; fu.notification("Walkspeed set."); ws=tonumber(speed) or 16 end}})
	local walkspeedInDeCrement=2
	table.insert(module,{Type="Button",Args={"Increase walkspeed", function(Self) local lp=game:GetService("Players").LocalPlayer; local char=lp.Character; if not char then fu.notification("No character!") return end; local hu=char:FindFirstChildOfClass("Humanoid"); if not hu then fu.notification("No humanoid on your character..?") return end; ws=ws+walkspeedInDeCrement; hu.WalkSpeed=hu.WalkSpeed+walkspeedInDeCrement; fu.notification("Walkspeed is now "..hu.WalkSpeed) end}})
	table.insert(module,{Type="Button",Args={"Decrease walkspeed", function(Self) local lp=game:GetService("Players").LocalPlayer; local char=lp.Character; if not char then fu.notification("No character!") return end; local hu=char:FindFirstChildOfClass("Humanoid"); if not hu then fu.notification("No humanoid on your character..?") return end; ws=ws-walkspeedInDeCrement; hu.WalkSpeed=hu.WalkSpeed-walkspeedInDeCrement; fu.notification("Walkspeed is now "..hu.WalkSpeed) end}})
	table.insert(module,{Type="Input",Args={"Walkspeed increment (How big each increase/decrease is)","Set",function(Self,input) walkspeedInDeCrement=tonumber(input) or 2; if not tonumber(input) then fu.notification("Not a number. Setting to default (2).") end; fu.notification("Set walkspeed increment to "..walkspeedInDeCrement) end}})
	table.insert(module,{Type="Input",Args={"FOV change","Set FOV",function(Self,tofov) if not tonumber(tofov) then fu.notification("Not a number. Setting to default.") end; ts:Create(workspace.CurrentCamera, TweenInfo.new(1, Enum.EasingStyle.Cubic, Enum.EasingDirection.Out), {FieldOfView=tonumber(tofov) or 70}):Play(); fov=tonumber(tofov) or 70 end}})
	table.insert(module,{Type="Toggle",Args={"Loop walkspeed and FOV", function(Self,state) loopfovandws=state end}})
	if uis.KeyboardEnabled and uis.MouseEnabled then table.insert(module,{Type="Toggle",Args={"CTRL+Click Teleport", function(Self,state) ctrlclicktp=state end}}) end
	table.insert(module,{Type="Text",Args={"Teleports"}})
	local function gotoPlayer(targetPlayerName) local targetPlayer=Players:FindFirstChild(getPlayer(targetPlayerName, game.Players.LocalPlayer)) if targetPlayer then local character=targetPlayer.Character if character and character:FindFirstChild("HumanoidRootPart") then local targetPosition=character.HumanoidRootPart.Position; local playerCharacter=Players.LocalPlayer.Character; if playerCharacter and playerCharacter:FindFirstChild("HumanoidRootPart") then playerCharacter.HumanoidRootPart.CFrame=CFrame.new(targetPosition+Vector3.new(0,5,0)) end end else print("Player '"..targetPlayerName.."' not found.") end end
	table.insert(module,{Type="Input",Args={"Enter player's name","Teleport",function(Self,text) gotoPlayer(text) end}})
	local spectateLoop=nil; table.insert(module,{Type="Button",Args={"Spectate players", function(Self) local listofplayers=game.Players:GetPlayers(); local currentlyViewing=1; local currentPlayer=listofplayers[currentlyViewing]; if not currentPlayer then return end; workspace.CurrentCamera.CameraSubject=currentPlayer.Character.Humanoid; spectateLoop=task.spawn(function() while true do fu.dialog("Spectating...", "Now spectating: "..workspace.CurrentCamera.CameraSubject.Parent.Name, {"Previous","Stop","Next"}); local action=fu.waitfordialog(); if action=="Stop" then fu.closedialog(); workspace.CurrentCamera.CameraSubject=game.Players.LocalPlayer.Character.Humanoid; task.cancel(spectateLoop); break elseif action=="Next" then currentlyViewing=currentlyViewing+1; if currentlyViewing>#listofplayers then currentlyViewing=1 end; currentPlayer=listofplayers[currentlyViewing]; if not currentPlayer then return end; workspace.CurrentCamera.CameraSubject=currentPlayer.Character.Humanoid elseif action=="Previous" then currentlyViewing=currentlyViewing-1; if currentlyViewing<1 then currentlyViewing=#listofplayers end; currentPlayer=listofplayers[currentlyViewing]; if not currentPlayer then return end; workspace.CurrentCamera.CameraSubject=currentPlayer.Character.Humanoid end end end) end}})
	table.insert(module,{Type="Text",Args={"Aim locking"}})
	local aimlockrscon; local target
	table.insert(module,{Type="Input",Args={"Target player","Set target",function(Self,input) if not Players:FindFirstChild(getPlayer(input, game.Players.LocalPlayer)) then fu.notification("Player not found.") return end; fu.notification("Target is set to "..Players:FindFirstChild(getPlayer(input, game.Players.LocalPlayer)).Name); target=Players:FindFirstChild(getPlayer(input, game.Players.LocalPlayer)) end}})
	local aimlock=false; local cam=workspace.CurrentCamera
	table.insert(module,{Type="Button",Args={"Aim lock", function(Self) if aimlock then return end; if aimlockrscon then aimlockrscon:Disconnect() end; if not target then fu.notification("Set a target first.") return end; aimlockrscon=rs.RenderStepped:Connect(function() if not target then fu.notification("No valid target."); aimlockrscon:Disconnect(); return end; if not target.Character then return end; if not target.Character:FindFirstChild("HumanoidRootPart") then return end; cam.CFrame=CFrame.new(cam.CFrame.Position, target.Character:FindFirstChild("HumanoidRootPart").Position) end); aimlock=true; fu.notification("Aim lock is now on.") end}})
	table.insert(module,{Type="Button",Args={"Unaim lock", function(Self) if not aimlock then return end; aimlock=false; if aimlockrscon then aimlockrscon:Disconnect() end; fu.notification("Aim lock is now off.") end}})
	table.insert(module,{Type="Text",Args={"Fling"}})
	local playerToFling; table.insert(module,{Type="Input",Args={"Target fling player","Set target",function(Self,input) if not Players:FindFirstChild(getPlayer(input, game.Players.LocalPlayer)) then fu.notification("Player not found.") return end; fu.notification("Target is set to "..Players:FindFirstChild(getPlayer(input, game.Players.LocalPlayer)).Name); playerToFling=Players:FindFirstChild(getPlayer(input, game.Players.LocalPlayer)) end}})
	local antiFling=false
	table.insert(module,{Type="ButtonGrid",Args={1,{
		Fling=function(Self) if not playerToFling then fu.notification("You need to target a player to fling.") return end; if not Players:FindFirstChild(playerToFling.Name) then fu.notification("You need to target a player to fling.") return end; if antiFling then fu.notification("Turn off anti-fling to use fling.") return end; local player=game.Players.LocalPlayer; local mouse=player:GetMouse(); local Targets={playerToFling}; local Players=game:GetService("Players"); local Player=Players.LocalPlayer; local AllBool=false; local SkidFling=function(TargetPlayer) local Character=Player.Character; local Humanoid=Character and Character:FindFirstChildOfClass("Humanoid"); local RootPart=Humanoid and Humanoid.RootPart; local TCharacter=TargetPlayer.Character; local THumanoid; local TRootPart; local THead; local Accessory; local Handle; if TCharacter:FindFirstChildOfClass("Humanoid") then THumanoid=TCharacter:FindFirstChildOfClass("Humanoid") end; if THumanoid and THumanoid.RootPart then TRootPart=THumanoid.RootPart end; if TCharacter:FindFirstChild("Head") then THead=TCharacter.Head end; if TCharacter:FindFirstChildOfClass("Accessory") then Accessory=TCharacter:FindFirstChildOfClass("Accessory") end; if Accessory and Accessory:FindFirstChild("Handle") then Handle=Accessory.Handle end; if Character and Humanoid and RootPart then if RootPart.Velocity.Magnitude<50 then getgenv().OldPos=RootPart.CFrame end; if THumanoid and THumanoid.Sit and not AllBool then end; if THead then if THead.Velocity.Magnitude>500 then fu.dialog("Player flung","Player is already flung. Fling again?",{"Fling again","No"}); if fu.waitfordialog()=="No" then return fu.closedialog() end; fu.closedialog() end elseif not THead and Handle then if Handle.Velocity.Magnitude>500 then fu.dialog("Player flung","Player is already flung. Fling again?",{"Fling again","No"}); if fu.waitfordialog()=="No" then return fu.closedialog() end; fu.closedialog() end end; if THead then workspace.CurrentCamera.CameraSubject=THead elseif not THead and Handle then workspace.CurrentCamera.CameraSubject=Handle elseif THumanoid and TRootPart then workspace.CurrentCamera.CameraSubject=THumanoid end; if not TCharacter:FindFirstChildWhichIsA("BasePart") then return end; local FPos=function(BasePart,Pos,Ang) RootPart.CFrame=CFrame.new(BasePart.Position)*Pos*Ang; Character:SetPrimaryPartCFrame(CFrame.new(BasePart.Position)*Pos*Ang); RootPart.Velocity=Vector3.new(9e7,9e7*10,9e7); RootPart.RotVelocity=Vector3.new(9e8,9e8,9e8) end; local SFBasePart=function(BasePart) local TimeToWait=2; local Time=tick(); local Angle=0; repeat if RootPart and THumanoid then if BasePart.Velocity.Magnitude<50 then Angle=Angle+100; FPos(BasePart, CFrame.new(0,1.5,0)+THumanoid.MoveDirection*BasePart.Velocity.Magnitude/1.25, CFrame.Angles(math.rad(Angle),0,0)); task.wait(); FPos(BasePart, CFrame.new(0,-1.5,0)+THumanoid.MoveDirection*BasePart.Velocity.Magnitude/1.25, CFrame.Angles(math.rad(Angle),0,0)); task.wait(); FPos(BasePart, CFrame.new(2.25,1.5,-2.25)+THumanoid.MoveDirection*BasePart.Velocity.Magnitude/1.25, CFrame.Angles(math.rad(Angle),0,0)); task.wait(); FPos(BasePart, CFrame.new(-2.25,-1.5,2.25)+THumanoid.MoveDirection*BasePart.Velocity.Magnitude/1.25, CFrame.Angles(math.rad(Angle),0,0)); task.wait(); FPos(BasePart, CFrame.new(0,1.5,0)+THumanoid.MoveDirection, CFrame.Angles(math.rad(Angle),0,0)); task.wait(); FPos(BasePart, CFrame.new(0,-1.5,0)+THumanoid.MoveDirection, CFrame.Angles(math.rad(Angle),0,0)); task.wait() else FPos(BasePart, CFrame.new(0,1.5,THumanoid.WalkSpeed), CFrame.Angles(math.rad(90),0,0)); task.wait(); FPos(BasePart, CFrame.new(0,-1.5,-THumanoid.WalkSpeed), CFrame.Angles(0,0,0)); task.wait(); FPos(BasePart, CFrame.new(0,1.5,THumanoid.WalkSpeed), CFrame.Angles(math.rad(90),0,0)); task.wait(); FPos(BasePart, CFrame.new(0,1.5,TRootPart.Velocity.Magnitude/1.25), CFrame.Angles(math.rad(90),0,0)); task.wait(); FPos(BasePart, CFrame.new(0,-1.5,-TRootPart.Velocity.Magnitude/1.25), CFrame.Angles(0,0,0)); task.wait(); FPos(BasePart, CFrame.new(0,1.5,TRootPart.Velocity.Magnitude/1.25), CFrame.Angles(math.rad(90),0,0)); task.wait(); FPos(BasePart, CFrame.new(0,-1.5,0), CFrame.Angles(math.rad(90),0,0)); task.wait(); FPos(BasePart, CFrame.new(0,-1.5,0), CFrame.Angles(0,0,0)); task.wait(); FPos(BasePart, CFrame.new(0,-1.5,0), CFrame.Angles(math.rad(-90),0,0)); task.wait(); FPos(BasePart, CFrame.new(0,-1.5,0), CFrame.Angles(0,0,0)); task.wait() end else break end until BasePart.Velocity.Magnitude>500 or BasePart.Parent~=TargetPlayer.Character or TargetPlayer.Parent~=Players or TargetPlayer.Character~=TCharacter or THumanoid.Sit or Humanoid.Health<=0 or tick()>Time+TimeToWait end; workspace.FallenPartsDestroyHeight=0/0; local BV=Instance.new("BodyVelocity"); BV.Name="EpixVel"; BV.Parent=RootPart; BV.Velocity=Vector3.new(9e8,9e8,9e8); BV.MaxForce=Vector3.new(1/0,1/0,1/0); Humanoid:SetStateEnabled(Enum.HumanoidStateType.Seated,false); if TRootPart and THead then if (TRootPart.CFrame.p-THead.CFrame.p).Magnitude>5 then SFBasePart(THead) else SFBasePart(TRootPart) end elseif TRootPart and not THead then SFBasePart(TRootPart) elseif not TRootPart and THead then SFBasePart(THead) elseif not TRootPart and not THead and Accessory and Handle then SFBasePart(Handle) else fu.notification("Can't find a proper part of target player to fling.") end; BV:Destroy(); Humanoid:SetStateEnabled(Enum.HumanoidStateType.Seated,true); workspace.CurrentCamera.CameraSubject=Humanoid; repeat RootPart.CFrame=getgenv().OldPos*CFrame.new(0,.5,0); Character:SetPrimaryPartCFrame(getgenv().OldPos*CFrame.new(0,.5,0)); Humanoid:ChangeState("GettingUp"); table.foreach(Character:GetChildren(), function(_,x) if x:IsA("BasePart") then x.Velocity,x.RotVelocity=Vector3.new(),Vector3.new() end end); task.wait() until (RootPart.Position-getgenv().OldPos.p).Magnitude<25; workspace.FallenPartsDestroyHeight=getgenv().FPDH else fu.notification("No valid character of said target player. May have died.") end end; SkidFling(Targets[1]) end,
	}}})
	local antiFlingLastPos=Vector3.zero; local flingNeutralizerCon; local flingDetectionCon; local detectedPlayers={}
	table.insert(module,{Type="Toggle",Args={"Anti-fling", function(Self,state) antiFling=state; if state then fu.notification("Anti-fling activated."); flingDetectionCon=rs.Heartbeat:Connect(function() for _,pl in ipairs(game:GetService("Players"):GetPlayers()) do if pl.Character:IsDescendantOf(workspace) then if pl.Character.PrimaryPart.AssemblyAngularVelocity.Magnitude>50 or pl.Character.PrimaryPart.AssemblyLinearVelocity.Magnitude>100 then if not detectedPlayers[pl.Name] then fu.notification("A flinger has been detected with the name "..pl.Name.."!"); detectedPlayers[pl.Name]=true end; for _,p in ipairs(pl.Character:GetDescendants()) do if p:IsA("BasePart") then p.CanCollide=false; p.AssemblyAngularVelocity=Vector3.zero; p.AssemblyLinearVelocity=Vector3.zero; p.CustomPhysicalProperties=PhysicalProperties.new(0,0,0) end end end end end end); flingNeutralizerCon=rs.Heartbeat:Connect(function() if game.Players.LocalPlayer.Character and game.Players.LocalPlayer.Character.PrimaryPart then if game.Players.LocalPlayer.Character.PrimaryPart.AssemblyLinearVelocity.Magnitude>250 or game.Players.LocalPlayer.Character.PrimaryPart.AssemblyAngularVelocity.Magnitude>250 then fu.notification("You were flung. Neutralizing velocity!"); game.Players.LocalPlayer.Character.PrimaryPart.AssemblyLinearVelocity=Vector3.zero; game.Players.LocalPlayer.Character.PrimaryPart.AssemblyAngularVelocity=Vector3.zero; if antiFlingLastPos~=Vector3.zero then game.Players.LocalPlayer.Character.PrimaryPart.CFrame=CFrame.new(antiFlingLastPos) end else antiFlingLastPos=game.Players.LocalPlayer.Character.PrimaryPart.Position end end end) else flingDetectionCon:Disconnect(); flingNeutralizerCon:Disconnect(); detectedPlayers={}; fu.notification("Anti-fling deactivated.") end end}})
	table.insert(module,{Type="Text",Args={"Miscellaneous"}})
	table.insert(module,{Type="Button",Args={"Anti AFK detection", function(Self) local pl=game.Players.LocalPlayer; if getconnections then for _,connection in pairs(getconnections(pl.Idled)) do if connection["Disable"] then connection["Disable"](connection) elseif connection["Disconnect"] then connection["Disconnect"](connection) end end else pl.Idled:Connect(function() game:GetService("VirtualUser"):CaptureController(); game:GetService("VirtualUser"):ClickButton2(Vector2.new()) end) end end}})
	pcall(function() if game:GetService("CoreGui"):FindFirstChild("DeltaIcon") then table.insert(module,{Type="Toggle",Args={"Hide Delta Icon", function(Self,state) game:GetService("CoreGui"):FindFirstChild("DeltaIcon").Enabled=state end}}) end end)
	table.insert(module,{Type="Button",Args={"Hide ASTROSTAR", function(Self) if uis.KeyboardEnabled then ts:Create(script.Parent.Menu.UIScale, TweenInfo.new(0.6, Enum.EasingStyle.Cubic, Enum.EasingDirection.Out), {Scale=0}):Play(); hidden=true; fu.notification("Press CTRL+SHIFT+Y to bring back the menu.") elseif uis.AccelerometerEnabled then ts:Create(script.Parent.Menu.UIScale, TweenInfo.new(0.6, Enum.EasingStyle.Cubic, Enum.EasingDirection.Out), {Scale=0}):Play(); hidden=true; fu.notification("Shake your device to bring back the menu.") else fu.notification("Can't hide ASTROSTAR!") end end}})
	table.insert(module,{Type="Button",Args={"FPS Boost", function(Self) fu.dialog("FPS boosting","FPS boosting can have unpredictable effects. You may instead lag more using this!",{"FPS boost anyway","Nevermind"}); local result=fu.waitfordialog(); fu.closedialog(); if result=="FPS boost anyway" then local Terrain=workspace:FindFirstChildOfClass('Terrain'); Terrain.WaterWaveSize=0; Terrain.WaterWaveSpeed=0; Terrain.WaterReflectance=0; Terrain.WaterTransparency=0; game.Lighting.GlobalShadows=false; game.Lighting.FogEnd=9e9; pcall(function() settings().Rendering.QualityLevel=1 end); for i,v in pairs(game:GetDescendants()) do if v:IsA("Part") or v:IsA("UnionOperation") or v:IsA("MeshPart") or v:IsA("CornerWedgePart") or v:IsA("TrussPart") then v.Material="Plastic"; v.Reflectance=0 elseif v:IsA("Decal") then v.Transparency=1 elseif v:IsA("ParticleEmitter") or v:IsA("Trail") then v.Lifetime=NumberRange.new(0) elseif v:IsA("Explosion") then v.BlastPressure=1; v.BlastRadius=1 end end; for i,v in pairs(game.Lighting:GetDescendants()) do if v:IsA("BlurEffect") or v:IsA("SunRaysEffect") or v:IsA("ColorCorrectionEffect") or v:IsA("BloomEffect") or v:IsA("DepthOfFieldEffect") then v.Enabled=false end end; workspace.DescendantAdded:Connect(function(child) task.spawn(function() if child:IsA('ForceField') then rs.Heartbeat:Wait(); child:Destroy() elseif child:IsA('Sparkles') then rs.Heartbeat:Wait(); child:Destroy() elseif child:IsA('Smoke') or child:IsA('Fire') then rs.Heartbeat:Wait(); child:Destroy() end end) end) end end}})
	local clip=true; local nocliploop
	table.insert(module,{Type="ButtonGrid",Args={2,{
		Noclip=function() clip=false; nocliploop=rs.Stepped:Connect(function() if clip==false and game.Players.LocalPlayer.Character~=nil then for _,child in pairs(game.Players.LocalPlayer.Character:GetDescendants()) do if child:IsA("BasePart") and child.CanCollide==true then child.CanCollide=false end end end end) end,
		Reclip=function() if clip then return end; clip=true; nocliploop:Disconnect(); fu.notification("Reclipping may need you to reset your character.") end,
	}}})
	table.insert(module,{Type="Text",Args={"Other"}})
	table.insert(module,{Type="Button",Args={"Get ping", function(Self) fu.notification(game.Players.LocalPlayer:GetNetworkPing()*1000) end}})
	table.insert(module,{Type="Button",Args={"Open developer console (debugging)", function(Self) game.StarterGui:SetCore("DevConsoleVisible",true) end}})
	function themeSerialize(data) local function s(v) local t=typeof(v); if t=="number" or t=="string" or t=="boolean" then return v; elseif t=="Color3" then return {__type="Color3", r=math.floor(v.R*255+0.5), g=math.floor(v.G*255+0.5), b=math.floor(v.B*255+0.5)}; elseif t=="EnumItem" then return {__type="EnumItem", enumType=v.EnumType.Name, name=v.Name}; elseif t=="ColorSequence" then local kp={}; for _,k in ipairs(v.Keypoints) do table.insert(kp,{t=k.Time, v={r=math.floor(k.Value.R*255+0.5), g=math.floor(k.Value.G*255+0.5), b=math.floor(k.Value.B*255+0.5)}}) end; return {__type="ColorSequence", keypoints=kp}; elseif t=="table" then local out={}; for k,val in pairs(v) do if k~="font" then out[k]=s(val) end end; return out; else error("Unsupported type: "..t) end end; return s(data) end
	function themeDeserialize(data) local function d(v) if typeof(v)~="table" then return v end; if v.__type=="Color3" then return Color3.fromRGB(v.r,v.g,v.b); elseif v.__type=="EnumItem" then local e=Enum[v.enumType]; return e and e[v.name] or nil; elseif v.__type=="ColorSequence" then local kps={}; for _,k in ipairs(v.keypoints) do table.insert(kps, ColorSequenceKeypoint.new(k.t, Color3.fromRGB(k.v.r,k.v.g,k.v.b))) end; return ColorSequence.new(kps); else local out={}; for k,val in pairs(v) do out[k]=d(val) end; return out end end; return d(data) end
	table.insert(module,{Type="Text",Args={"Theme"}})
	local function loadThemeFromSave(last) if not last then task.wait(1) else task.wait(0.2) end; if ASTROSTARPointSave:get("ASTROSTARGlobal_themeCode") then local themeObjectImport=themeDeserialize(https:JSONDecode(fu.from_base64(ASTROSTARPointSave:get("ASTROSTARGlobal_themeCode")))); theme:setColorTable(themeObjectImport); theme:init(getgenv().ASTROSTAR); fu.setTheme(themeObjectImport); fu.refreshlist(); fu.refresharea(); if not last then loadThemeFromSave(true) end end end
	table.insert(module,{Type="Input",Args={"Theme code","Apply",function(obj,value) local themeObjectImport=themeDeserialize(https:JSONDecode(fu.from_base64(value))); theme:setColorTable(themeObjectImport); theme:init(getgenv().ASTROSTAR); fu.setTheme(themeObjectImport); fu.refreshlist(); fu.refresharea(); fu.notification("Successfully applied theme!"); ASTROSTARPointSave:set("ASTROSTARGlobal_themeCode",value) end}})
	table.insert(module,{Type="Button",Args={"Reload theme", function() theme:init(getgenv().ASTROSTAR); fu.refreshlist(); fu.refresharea() end}})
	table.insert(module,{Type="Button",Args={"Delete theme from save", function() ASTROSTARPointSave:remove("ASTROSTARGlobal_themeCode"); fu.notification("Theme will not be restored on the next executes.") end}})
...
local function DSZIHQM_routine()
	...
	local s,e=pcall(function()
		if get_hidden_gui or gethui then local hiddenUI=get_hidden_gui or gethui; script.Parent.Name=randomString(); script.Parent.Parent=hiddenUI()
		elseif (not is_sirhurt_closure) and (syn and syn.protect_gui) then script.Parent.Name=randomString(); syn.protect_gui(script.Parent); script.Parent.Parent=COREGUI
		elseif COREGUI:FindFirstChild('RobloxGui') then script.Parent.Parent=COREGUI.RobloxGui end
	end)
...
local function EXEKBZ_routine()
	...
	module.players=false; module.items=false; module.generators=false; module.priotizingmedkit=false; module.infinitestamina=false; module.noCooldownSolve=false
	local solveGeneratorCooldown=false
	local conns={}
	local function addPlayer(p) espcontainer:Add(p,{AccentColor=Color3.new(0.149,1,0), ArrowShow=false, ShowLabel=false, GroupName="players"}) end
	local function addKiller(k) espcontainer:Add(k,{AccentColor=Color3.new(1,0,0), ArrowShow=true, ArrowMinDistance=999999, ArrowSize=UDim2.new(0,40,0,40), LabelText="Killer", ShowLabel=true, GroupName="players"}) end
	local function addItem(i) local cfg={AccentColor=Color3.new(0.0313725,0.917647,1), ArrowShow=false, ShowLabel=false, GroupName="items"}; if module.priotizingmedkit and i.Name=="Medkit" then fu.notification("Medkit spawned!"); cfg={AccentColor=Color3.new(1,1,1), ArrowShow=true, ArrowMinDistance=999999, ArrowSize=UDim2.new(0,40,0,40), LabelText="Medkit", ShowLabel=true, GroupName="items"} end; espcontainer:Add(i,cfg) end
	local function addGenerator(g) local progVal=g:FindFirstChild("Progress"); local progress=(progVal and progVal.Value) or 0; local showLabel=true; local showArrow=true; local accentColor=Color3.new(0.984,1,0); local labelText=tostring(math.floor(progress)).."%"; if progress>=100 then showLabel=false; showArrow=false; accentColor=Color3.new(0,0.5,0) end; espcontainer:Add(g,{AccentColor=accentColor, ArrowShow=showArrow, ArrowMinDistance=999999, ArrowSize=UDim2.new(0,20,0,20), ShowLabel=showLabel, LabelText=labelText, GroupName="generators"}); if progVal then progVal.Changed:Connect(function(new) espcontainer:Remove(g); addGenerator(g) end) end end
	local function getClosestGenerator() local char=game.Players.LocalPlayer.Character; if not char or not char.PrimaryPart then return nil end; local root=char.PrimaryPart; local closest,shortestDist=nil,math.huge; local map=workspace.Map:FindFirstChild("Ingame"):FindFirstChild("Map"); if map then for _,obj in ipairs(map:GetChildren()) do if obj.Name=="Generator" and obj:IsA("Model") and obj.PrimaryPart then local dist=(root.Position-obj.PrimaryPart.Position).Magnitude; if dist<shortestDist then closest=obj; shortestDist=dist end end end end; return closest end
	local function removeIndicator(obj) espcontainer:Remove(obj) end
	local function listen(parent,signal,callback) local conn=signal:Connect(callback); table.insert(conns,conn); return conn end
	local function setupAutoUpdate() local s,k=workspace.Players:FindFirstChild("Survivors"), workspace.Players:FindFirstChild("Killers"); if s then listen(s,s.ChildAdded,function(c) if module.players then addPlayer(c) end end); listen(s,s.ChildRemoved,function(c) if module.players then removeIndicator(c) end end) end; if k then listen(k,k.ChildAdded,function(c) if module.players then addKiller(c) end end); listen(k,k.ChildRemoved,function(c) if module.players then removeIndicator(c) end end) end; local ingame=workspace.Map:FindFirstChild("Ingame"); if ingame then listen(ingame,ingame.ChildAdded,function(c) if module.items and c.Name~="Map" then addItem(c) end end); listen(ingame,ingame.ChildRemoved,function(c) if module.items and c.Name~="Map" then removeIndicator(c) end end); local map=ingame:FindFirstChild("Map"); if map then listen(map,map.ChildAdded,function(c) if module.generators and c.Name=="Generator" then addGenerator(c) end end); listen(map,map.ChildRemoved,function(c) if module.generators and c.Name=="Generator" then removeIndicator(c) end end) end end end
	function module.clear() for _,c in ipairs(conns) do c:Disconnect() end; table.clear(conns); espcontainer:ClearAllGroups() end
	task.spawn(function() local sprintingModule=require(game:GetService("ReplicatedStorage").Systems.Character.Game.Sprinting); while task.wait(1) do if module.infinitestamina then sprintingModule.StaminaLossDisabled=true end end end)
	table.insert(module,{Type="Text",Args={"ESPs"}})
	table.insert(module,{Type="ButtonGrid",Toggleable=true,Args={3,{
		Players=function(Self) module.players=not module.players; if module.players then local survivors=workspace.Players:FindFirstChild("Survivors"); if survivors then for _,p in ipairs(survivors:GetChildren()) do addPlayer(p) end end; local killers=workspace.Players:FindFirstChild("Killers"); if killers then local killer=killers:GetChildren()[1]; if killer then addKiller(killer) end end else espcontainer:RemoveGroup("players") end end,
		Items=function(Self) module.items=not module.items; if module.items then local ingame=workspace.Map:FindFirstChild("Ingame"); if ingame then for _,i in ipairs(ingame:GetChildren()) do if i.Name~="Map" then addItem(i) end end end else espcontainer:RemoveGroup("items") end end,
		Generators=function(Self) module.generators=not module.generators; if module.generators then local ingame=workspace.Map:FindFirstChild("Ingame"); if ingame then local map=ingame:FindFirstChild("Map"); if map then for _,g in ipairs(map:GetChildren()) do if g.Name=="Generator" then addGenerator(g) end end end end else espcontainer:RemoveGroup("generators") end end,
	}}})
	table.insert(module,{Type="Toggle",Args={"Show medkit in item ESP", function(Self,state) module.priotizingmedkit=state; espcontainer:RemoveGroup("items"); if module.items then local ingame=workspace.Map:FindFirstChild("Ingame"); if ingame then for _,i in ipairs(ingame:GetChildren()) do if i.Name~="Map" then addItem(i) end end end end end}})
	table.insert(module,{Type="Text",Args={"Tools"}})
	table.insert(module,{Type="Button",Args={"Finish generator puzzle", function(Self) if solveGeneratorCooldown then fu.notification("Cooldown for 4 seconds!") return end; solveGeneratorCooldown=not module.noCooldownSolve; if getClosestGenerator() then getClosestGenerator().Remotes.RE:FireServer() end; task.wait(4); solveGeneratorCooldown=false end}})
	table.insert(module,{Type="Text",Args={"You will automatically solve the closest generator."}})
	table.insert(module,{Type="Toggle",Args={"Bypass puzzle solver cooldown", function(Self,state) module.noCooldownSolve=state end}})
	...
	table.insert(module,{Type="Toggle",Args={"Infinite stamina", function(Self,state) module.infinitestamina=state; if not state then local sprintingModule=require(game:GetService("ReplicatedStorage").Systems.Character.Game.Sprinting); sprintingModule.StaminaLossDisabled=false end end}})
	table.insert(module,{Type="Input",Args={"Sprinting speed (Default = 26)","Set sprint speed",function(Self,text) local sprintingModule=require(game:GetService("ReplicatedStorage").Systems.Character.Game.Sprinting); sprintingModule.SprintSpeed=tonumber(text) or 26 end}})
	table.insert(module,{Type="Input",Args={"Stamina gain rate (Default = 20)","Set stamina gain",function(Self,text) local sprintingModule=require(game:GetService("ReplicatedStorage").Systems.Character.Game.Sprinting); sprintingModule.StaminaGain=tonumber(text) or 20 end}})
	setupAutoUpdate()
	repeat task.wait() until getgenv().Modules
	getgenv().Modules[4]=module
end
