...
do
	-- Module: StarterGui.ASTROSTAR.FUNCTIONS
	...
	local function module_script()
		...
		function FUNCTIONSmodule.getTheme()
			if getgenv then
				return getgenv().ASTROSTAR_THEME or FUNCTIONSmodule.theme
			else
		...
		function FUNCTIONSmodule.setTheme(t)
			...
			if getgenv then getgenv().ASTROSTAR_THEME = t end
		end
		...
		function FUNCTIONSmodule.createFloatingButton(item,button,buttonname,fromload)
			if not getgenv().ASTROSTAR.FloatingButtons:FindFirstChild(string.gsub(buttonname, "_", "")) then
				...
				if not fromload then
					ASTROSTARPointSave:set(string.gsub(buttonname, "_", ""), udim2Serializer(UDim2.fromOffset(125, 90)) .. "|" .. udim2Serializer(UDim2.fromOffset(200,50)) .. "|true|true")
				end
			...
			for _, module in ipairs(getgenv().Modules) do
				for _, item in ipairs(module) do
					if item["Type"] == "Button" then
						...
						local saved = ASTROSTARPointSave:get(key)
						if saved then
		...
		function FUNCTIONSmodule.refreshlist()
			...
			for i, module in ipairs(dense) do
				...
				if not success then
					warn(("[ASTROSTAR] Error loading module %q: %s"):format(module.Name, err))
				end
			end
		end
...
do
	-- Module: StarterGui.ASTROSTAR.DraggableObject
	local script = Instance.new("ModuleScript")
...
do
	-- Module: StarterGui.ASTROSTAR.ClickAndHold
	local script = Instance.new("ModuleScript")
...
do
	-- Module: StarterGui.ASTROSTAR.Spring
	local script = Instance.new("ModuleScript")
	script.Name = "Spring"
	script.Parent = Converted["_ASTROSTAR"]
	local function module_script()
...
do
	-- Module: StarterGui.ASTROSTAR.ESPIndicator
	local script = Instance.new("ModuleScript")
...
do
	-- Module: StarterGui.ASTROSTAR.Bezier
	local script = Instance.new("ModuleScript")
	script.Name = "Bezier"
	script.Parent = Converted["_ASTROSTAR"]
	local function module_script()
...
do
	-- Module: StarterGui.ASTROSTAR.PointSave
	local script = Instance.new("ModuleScript")
...
do
	-- Module: StarterGui.ASTROSTAR.Theme
	local script = Instance.new("ModuleScript")
	script.Name = "Theme"
	script.Parent = Converted["_ASTROSTAR"]
	local function module_script()
...
do
	-- Module: StarterGui.ASTROSTAR.FlyUtility
	local script = Instance.new("ModuleScript")
...
local function FKVEADO_routine()
...
local function KWZT_routine()
	...
	table.insert(module, {
		Type = "Text",
		Args = {"<font color='#FFFF00'>Another great script</font> by ASTRO STAR developers below!"}
	})
	table.insert(module, {
		Type = "Button",
	...
	table.insert(module, {
		Type = "Button",
		Args = {"Hide ASTROSTAR", function(Self)
			...
			elseif uis.AccelerometerEnabled then
				...
				fu.notification("Can't hide ASTROSTAR!")
			end
		end,}
	})
	table.insert(module, {
	...
	local function loadThemeFromSave(last)
		...
		if ASTROSTARPointSave:get("ASTROSTARGlobal_themeCode") then
			local themeObjectImport = themeDeserialize(https:JSONDecode(fu.from_base64(ASTROSTARPointSave:get("ASTROSTARGlobal_themeCode"))))
			theme:setColorTable(themeObjectImport)
			theme:init(getgenv().ASTROSTAR)
	...
	table.insert(module, {
		...
		Args = {
			...
			function(obj, value)
				...
				ASTROSTARPointSave:set("ASTROSTARGlobal_themeCode", value)
			end
		}
	})
	table.insert(module, {
	...
	table.insert(module, {
		...
		Args = {"Delete theme from save", function()
			ASTROSTARPointSave:remove("ASTROSTARGlobal_themeCode")
	...
	table.insert(module, {
		Type = "Toggle",
		Args = {"Hide ASTROSTAR+/Developer tags", function(_, state)
	...
	table.insert(module, {
		Type = "Dropdown",
		Args = {"how rizz is astrostar", {"very rizz", "no rizz"}, function(Self, item)
			print(item)
		end,}
	})
	repeat task.wait() until getgenv().Modules
...
local function ZUTHJ_routine()
	...
	if (module.gameId ~= game.GameId) then
		table.insert(module, {
			Type = "Text",
			Args = {"ASTROSTAR now includes an official <font color='#FFFF00'>Forsaken</font> module!"}
		})
		table.insert(module, {
			Type = "Text",
			Args = {"<font size='10'>To use, run ASTROSTAR on Forsaken.</font>"}
		})
		repeat task.wait() until getgenv().Modules
	...
	table.insert(module, {
		...
		Args = {"Finish generator puzzle", function(Self)
			...
			solveGeneratorCooldown = not module.noCooldownSolve
...
local function IJIOJM_routine()
	...
	local function getPredictedPosition(player, shootOffset)
		...
		elseif predictionAIEngine and not getgenv().ASTROSTARNetwork.predictPos then
			fu.notification("ASTROSTAR AI Engine is not available. Reverting to basic prediction.")
		end
			...
			Auto_Shoot_murderer = function()
				...
				if findSheriff() == localplayer and autoShooting then
					...
					repeat
						task.wait(0.1)
						local murderer = findMurderer() or findSheriffThatsNotMe()
						if not murderer then warn("[ASTROSTAR] > MM2 Autoshoot - No murderer.") continue end
						local murdererPosition = murderer.Character.HumanoidRootPart.Position
						local characterRootPart = localplayer.Character.HumanoidRootPart
	...
	table.insert(module, {
		...
		Args = {"Send Sheriff and Murderer names into chat", function(Self)
			...
			for _, textchannel in ipairs(textchannels) do
				...
				local message = string.format([[Murderer: %s | 		Sheriff: %s | 		<<ASTROSTAR>>]], murdName, sherName)
...
local function QXNY_routine()
	...
	script.Parent.MouseButton1Click:Connect(function()
		getgenv().ASTROSTARFUNCTIONS.ftToggleVisibility()
	end)
end

local function OLQBK_routine()
	local script = Instance.new("LocalScript")
	script.Name = "LocalScript"
	...
	script.Parent.MouseButton1Click:Connect(function()
		getgenv().ASTROSTARFUNCTIONS.ftToggleLock()
	end)
end

local function QUDG_routine()
	local script = Instance.new("LocalScript")
	script.Name = "LocalScript"
	...
	script.Parent.MouseButton1Click:Connect(function()
		getgenv().ASTROSTARFUNCTIONS.closeFinetuneFB()
	end)
end
